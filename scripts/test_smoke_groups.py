...
def test_dependency_audit_parses_pinned_sha() -> None:
    ...
    assert audit["pinned_ref"] == "f2379da6b3b7cfb24fe32614e7c50008c306fecb", audit
    assert audit["ref_kind"] == "commit", audit
    assert audit["is_pinned"] is True, audit
    assert audit["resolved_spedas_agent_kit_commit"] == "f2379da6b3b7cfb24fe32614e7c50008c306fecb", audit
...
def test_dependency_audit_direct_ref_with_ssh_userinfo_and_ref() -> None:
    ...
    server = {
        "args": [
            "--with", "mcp>=1.26.0,<2",
            "--from", "git+ssh://git@github.com/spedas/spedas_agent_kit.git@f2379da6b3b7cfb24fe32614e7c50008c306fecb",
            "spedas-agent-kit",
        ],
    }
    audit = smoke._parse_dependency_audit(server)
