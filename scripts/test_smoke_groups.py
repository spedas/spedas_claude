...
def test_dependency_audit_parses_pinned_sha() -> None:
    ...
    assert audit["pinned_ref"] == "8fcfc7dd0e6f01800f301590ed8213eb33683582", audit
    assert audit["ref_kind"] == "commit", audit
    assert audit["is_pinned"] is True, audit
    assert audit["resolved_spedas_agent_kit_commit"] == "8fcfc7dd0e6f01800f301590ed8213eb33683582", audit
...
def test_dependency_audit_direct_ref_with_ssh_userinfo_and_ref() -> None:
    ...
    server = {
        "args": [
            "--with", "mcp>=1.26.0,<2",
            "--from", "git+ssh://git@github.com/spedas/spedas_agent_kit.git@8fcfc7dd0e6f01800f301590ed8213eb33683582",
            "spedas-agent-kit",
        ],
    }
    audit = smoke._parse_dependency_audit(server)
