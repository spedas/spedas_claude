...
| Smoke hangs/times out on first run | `uvx` resolving the pinned `spedas_agent_kit` commit from GitHub with no/blocked network | allow network for first run; raise `--timeout`; pre-warm (`uvx --with 'mcp>=1.26.0,<2' --from git+https://github.com/spedas/spedas_agent_kit.git@48dc50d9c31ba608019c8ea3ac3d72ac2b5158b8 spedas-agent-kit --help`, matching `.mcp.json`) |
